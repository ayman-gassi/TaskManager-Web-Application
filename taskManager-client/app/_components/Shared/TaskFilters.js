import { FiSearch, FiX, FiCalendar, FiTag, FiUser, FiFlag } from 'react-icons/fi'
...
export default function TaskFilters({ onSearch, searchQuery = '', onClearSearch }) {
  ...
  const filterOptions = [
    { icon: <FiCalendar className="w-4 h-4" />, label: 'Due Date', color: 'text-blue-600' },
    { icon: <FiTag className="w-4 h-4" />, label: 'Priority', color: 'text-purple-600' },
    { icon: <FiFlag className="w-4 h-4" />, label: 'Status', color: 'text-orange-600' },
  ]

  return (
    <div className="bg-white px-6 py-3 border-y border-gray-200">
